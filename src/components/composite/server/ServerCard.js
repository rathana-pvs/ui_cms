...
import styles from '@/components/composite/server/ServerCard.module.scss'
...
export default function ServerCard({title, address, port, color, status, onContextMenu, onClick}) {
    ...
    return (
        <Tooltip placement="right" title={getToolTip()}>
            <div className={`${styles.card} ${status? styles.active_card: ""}`} onContextMenu={onContextMenu}>
                <Ripples during={800} color={'rgb(44, 62, 80, 0.4)'} onClick={onClick}>
                    <div className={styles.card__wrapper}>
                        <div className={styles.card__content}>
                            <div className={styles.card__title}>{title}</div>

                        </div>
                        <div className={`${styles.card__decor} ${card_decor[color]}`}>
