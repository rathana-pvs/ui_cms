import {Dropdown} from "antd";
import {
    PlusOutlined, ReloadOutlined,
} from "@ant-design/icons";
import {nanoid} from "nanoid";
import {useDispatch, useSelector} from "react-redux";
import {
    setBackupDB,
    setCheckDB,
    setCompactDB,
    setCopyDB,
    setDeleteDB, setLoading,
    setOptimizeDB, setParamDump, setPlanDump,
    setRenameDB, setRestoreDB
} from "@/store/dialogReducer";
import {startDatabaseAPI, stopDatabaseAPI} from "@/lib/api/cmApi";
import {setDatabases} from "@/store/treeReducer";
import {getDatabaseFormat} from "@/utils/navigation";
import React from "react";

export default function({node, event, open, onClose}) {
    const {activeServer, databases} = useSelector(state => state.treeReducer);
    const dispatch = useDispatch();
    const {clientX, clientY} = event;
    const menuItems = [
        {
            label: node.status === "inactive" ? "Start Database" : "Stop Database",
            key: nanoid(4),
            icon: <ReloadOutlined />,
            onClick: () => {
                dispatch(setLoading(true));
                if (node.status === "active") {
                    stopDatabaseAPI(activeServer, {dbname: node.title}).then(res => {
                        if(res.success) {
                            const newDatabase = databases.map(res=>{
                                if(res.key === node.key){
                                    return {...res, icon: <i className="fa-regular fa-database warning"/>}
                                }
                                return res;
                            })
                            dispatch(setDatabases(newDatabase))
                            dispatch(setLoading(false));

                        }
                    })
                }else {
                    startDatabaseAPI(activeServer, {dbname: node.title}).then(res => {
                        if(res.success) {
                            const newDatabase = databases.map(res=>{
                                if(res.key === node.key){

                                    return {...res, icon: <i className="fa-regular fa-database success"/>}
                                }
                                return res;
                            })
                            dispatch(setDatabases(newDatabase))
                            dispatch(setLoading(false));

                        }
                    })
                }
            }
        },
        {
            label: "Manage Database",
            disabled: true,
            key: nanoid(4),
            icon: <ReloadOutlined />,
            children: [
                {
                    label: "Database Unload",
                    key: nanoid(4),
                    disabled: true
                },
                {
                    label: "Database Load",
                    key: nanoid(4),
                    disabled: true
                },
                {
                    label: "Database Optimize",
                    key: nanoid(4),
                    disabled: node.status === "active",
                    onClick: () => {
                        dispatch(setOptimizeDB({open: true, node}));
                    }
                },
                {
                    label: "Compact Database",
                    key: nanoid(4),
                    onClick: () => {
                        dispatch(setCompactDB({open: true, node}));
                    }
                },
                {
                    label: "Check Database",
                    key: nanoid(4),
                    onClick: ()=>{
                        dispatch(setCheckDB({open: true, node}));
                    }
                },
                {
                    label: "Copy Database",
                    key: nanoid(4),
                    disabled: node.status === "active",
                    onClick: () => {
                        dispatch(setCopyDB({open: true, node}));
                    }
                },

                {
                    label: "Create Database",
                    key: nanoid(4),
                    disabled: true
                },
                {
                    label: "Rename Database",
                    key: nanoid(4),
                    disabled: node.status === "active",
                    onClick: ()=>{
                        dispatch(setRenameDB({open: true, node}));
                    }
                },
                {
                    label: "Backup Database",
                    key: nanoid(4),
                    onClick: ()=>{
                        dispatch(setBackupDB({open: true, node}));
                    }
                },
                {
                    label: "Restore DB",
                    key: nanoid(4),
                    disabled: node.status === "active",
                    onClick: ()=>{
                        dispatch(setRestoreDB({open: true, node}));
                    }
                },
                {
                    label: "Delete Database",
                    key: nanoid(4),
                    disabled: node.status === "active",
                    onClick: ()=>{
                        dispatch(setDeleteDB({open: true, node}));
                    }
                },
            ]
        },
        {
            label: "Database Info",
            key: nanoid(4),
            icon: <ReloadOutlined />,
            disabled: true,
            children: [
                {
                    label: "Lock Information",
                    key: nanoid(4),
                    disabled: true,
                },
                {
                    label: "Transaction Info",
                    key: nanoid(4),
                    disabled: true,
                },
                {
                    label: "Plan Dump",
                    key: nanoid(4),
                    disabled: node.status === "inactive",
                    onClick: ()=>{
                        dispatch(setPlanDump({open: true, node}));
                    }
                },
                {
                    label: "Param Dump",
                    key: nanoid(4),
                    disabled: node.status === "inactive",
                    onClick: ()=>{
                        dispatch(setParamDump({open: true, node}));
                    }
                },
                {
                    label: "OID Navigator",
                    key: nanoid(4),
                    disabled: true
                },
            ]
        },
        {
            label: "Properties",
            key: nanoid(4),
            icon: <ReloadOutlined />,
            disabled: true,
        },

    ]

    return (
        <>
            <Dropdown overlayStyle={{minWidth: 200}}  menu={{items: menuItems}}
                      trigger={["contextMenu"]}
                      onOpenChange={onClose}
                      open={open} placement="bottomLeft">
                <div style={{ position: "absolute", left: clientX, top: clientY, width: 0, height: 0 }} />
            </Dropdown>
        </>

    )
}